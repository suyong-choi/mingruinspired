...
class Mingrustack(nn.Module):
    def __init__(self, nlayers:int, input_size:int, hidden_size:int, dropout=0.05 ) -> None:
        ...
        self.stacks.append(nn.LayerNorm(hidden_size))
        self.stacks.append(nn.Dropout(dropout))
        self.stacks.append(nn.Linear(hidden_size, 1))
        self.stacks.append(nn.Sigmoid())
        pass
...
class Mingrustackbak(nn.Module):
    def __init__(self, nlayers:int, input_size:int, hidden_size:int, dropout=0.05 ) -> None:
        super(Mingrustack, self).__init__()

        self.stacks = nn.ModuleList([Mingrubare(input_size, hidden_size),nn.LayerNorm(hidden_size)])
        for _ in range(nlayers-1):
            self.stacks.append(nn.Dropout(dropout))
            self.stacks.append(nn.LayerNorm(hidden_size))
            self.stacks.append(Mingrubare(hidden_size, hidden_size))
